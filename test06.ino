 ...
 * 
 * This program reads humidity and temperature from AN2302(DHT22) sensors. 
 * Then the data & current time is displayed on the website built by ethernet module, 
 * and saved as CSV file in SD card.
 * If the temperature gets over TEMP_LIMIT, the error message is displayed.
 * If the module has a problem at the beginning of the program, it restarts automatically.
 * 
 * About the saved data
 *  - File directory: YYYYMMDD/L_(current time).csv
 *  - Data format
 *      1             ||  Time  | Humidity 1(%) | Temperature 1(deg C) | Humidity 2(%) | Temperature 2(deg C) | ...
 *      2             || YYMMDD |      50       |          25          |      50       |          25          | ...
 *      ...           ||   ...  |      ...      |          ...         |      ...      |          ...         | ...
 *      4320(approx.) || YYMMDD |      50       |          25          |      50       |          25          | ...
 * 
 * 
 */
...
//  Serial.println("SAVEWORKBOOKAS,WORKBOOK_PREV");
//  
//  Serial.println("CLEARSHEET");
//  Serial.print("LABEL,Time,");
//  for(int i=0 ;i<SENSOR_NUM; i++)
//    Serial.print((String) "Humidity "+(i+1)+" (%),Temperature "+(i+1)+" (deg C),");
//  Serial.println("");
  prev_millis=millis();
}
...
void loop() 
{
  ...
  if (pos) 
  {
    ...
    if (strncmp("GET / ", data, 6) == 0)
    {
      ...
      memcpy_P(ether.tcpOffset(), generalHeaderFormat, sizeof generalHeaderFormat); 
      ether.httpServerReply_with_flags(sizeof generalHeaderFormat - 1,TCP_FLAGS_ACK_V); 
      formatString_PROGMEM(buf, headerFormat, year(epoch), month(epoch), day(epoch), hour(epoch), minute(epoch), second(epoch));
      memcpy(ether.tcpOffset(), buf, strlen(buf)); // send a packet and not send the terminate flag
      ether.httpServerReply_with_flags(strlen(buf), TCP_FLAGS_ACK_V);

      for(int i=0; i<SENSOR_NUM; i++)
      {
        formatString_PROGMEM(buf, defaultFormat, i+1, t[i], h[i]);
        memcpy(ether.tcpOffset(), buf, strlen(buf)); // send a packet and not send the terminate flag
        ether.httpServerReply_with_flags(strlen(buf),TCP_FLAGS_ACK_V); 
      }
      ...
      for(int i=0; i<SENSOR_NUM; i++)
      {
        if(t[i]>=TEMP_LIMIT)
        {
          formatString_PROGMEM(buf, warningFormat, i+1);
          memcpy(ether.tcpOffset(), buf, strlen(buf)); // send a packet and not send the terminate flag
          ether.httpServerReply_with_flags(strlen(buf),TCP_FLAGS_ACK_V); 
        }
        
      }
      memcpy_P(ether.tcpOffset(), footerFormat, sizeof footerFormat); // send a last packet and send the terminate flag
      ether.httpServerReply_with_flags(sizeof footerFormat - 1,TCP_FLAGS_ACK_V|TCP_FLAGS_FIN_V); 
    }
    ...
    else  // Send error 404
    {
      ...
      memcpy_P(ether.tcpOffset(), generalHeaderFormat, sizeof generalHeaderFormat); 
      ether.httpServerReply_with_flags(sizeof generalHeaderFormat - 1,TCP_FLAGS_ACK_V); 
      memcpy_P(ether.tcpOffset(), err404Format, sizeof err404Format); 
      ether.httpServerReply_with_flags(sizeof err404Format, TCP_FLAGS_ACK_V);
      memcpy_P(ether.tcpOffset(), footerFormat, sizeof footerFormat); // send a last packet and send the terminate flag
      ether.httpServerReply_with_flags(sizeof footerFormat - 1,TCP_FLAGS_ACK_V|TCP_FLAGS_FIN_V); 
    }
